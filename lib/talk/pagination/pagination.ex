defmodule Talk.Pagination do
  @moduledoc "Functions for generating cursor paginated, GraphQL query results."

  import Ecto.Query

  alias Talk.Pagination.{Args, Edge, PageInfo, Result, Validations}
  ...
  ## Examples
      ...
      fetch_result(Talk.Repo, base_query, args)
  ...
  def fetch_result(base_query, args) do
    case validate_args(args) do
      {:ok, _} ->
        ...
        total_count = Talk.Repo.one(apply_count(base_query))

        {:ok, nodes, has_previous_page, has_next_page} =
          fetch_nodes(Talk.Repo, base_query, order_field, normalized_args)
  ...
  def map(%Result{edges: edges} = result, fun), do: %{result | edges: Enum.map(edges, fun)}
  ...
  defp normalize(%Args{last: last} = args) when is_nil(last), do: {args, false}
  ...
  defp start_cursor([edge | _]), do: edge.cursor

  defp end_cursor([]), do: nil

  defp end_cursor(edges), do: List.last(edges).cursor
